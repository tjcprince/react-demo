...
export default class Pagination extends Component {
  ...
  render() {
    ...
    buttons = [ < li key = "first" className = {
        !isNotFirst
          ? 'disabled'
          : null
      } > <a role="button"  tabIndex="0" onClick={firstHandler} aria-disabled={!isNotFirst} aria-label="First">
      ...
      <li key="prev" className={!isNotFirst ? 'disabled' : null}>
        <a role="button"  tabIndex="0"
          ...
          aria-label="Previous">
          <span className="fa fa-angle-left" aria-hidden="true" / > </a> < /li>,
    ].concat(buttons);
